...
namespace RobotServ
{
    public partial class Forum : System.Web.UI.Page
    {
        ...
        {
            string html = "";
            List<ThreadForum> thr = cc.ThreadForums.OrderByDescending(x => x.CreateDate).ToList();
            ...
            for (int c = 0; c < thr.Count; c++)
            {
                ...
                int totalReplies = cc.ThreadReplies.Count(x => x.ID_Thread == thr[c].ID);
                html += "        <tr>";
                html += "            <td>";
                html += "                " + thr[c].CreateDate.ToString("dd MMM")+ "";
                html += "            </td>";
                html += "            <td>";
                html += "               <a href=\"Thread.aspx?id="+thr[c].ID+"\"> <b>"+thr[c].Heading+"</b></a>";
                if (pl != null)
                {
        ...
        {
            Response.Redirect("StartThread.aspx");
        }
    }
}